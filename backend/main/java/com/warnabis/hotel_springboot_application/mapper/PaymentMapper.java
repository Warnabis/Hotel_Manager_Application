...
public class PaymentMapper {
    ...
    public PaymentResponseDto toResponseDto(Payment payment) {
        ...
        if (payment.getBookings() != null) {
            dto.setBookingIds(payment.getBookings().stream().map(b -> b.getId()).collect(Collectors.toList()));
        }
        if (payment.getServices() != null) {
            dto.setServiceIds(payment.getServices().stream().map(s -> s.getId()).collect(Collectors.toList()));
        }
        return dto;
    }
