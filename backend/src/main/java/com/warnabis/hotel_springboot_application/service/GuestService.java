...
public class GuestService {
    ...
    public GuestResponseDto create(GuestRequestDto request) {
        if (guestRepository.findByEmailIgnoreCase(request.getEmail()).isPresent()) {
            throw new com.warnabis.hotel_springboot_application.exception.ConflictException(
              "Гость с таким email уже существует");
        }
        Guest guest = guestMapper.toEntity(request);
        guest.setEmail(guest.getEmail().trim().toLowerCase());
        applyPassword(guest, request.getPassword());
    ...
    private void applyPassword(Guest guest, String password) {
